...
function populate_details(returned_data) {
  ...
  for (var k in returned_data) {
    if (isDict(returned_data[k])) {
      populate_details(returned_data[k]);
    } else {
      details_table.row.add([k, returned_data[k]]).draw(false);
    }
  }
}
  ...
  $.confirmModal('Are you sure to delete this?', function (el) {
    ...
    $.ajax({
      ...
      success: function (returned_data) {
        ...
        var details_table = $("#details_table").DataTable();
        details_table.clear().draw();
        $("#get_all_windows").click()
